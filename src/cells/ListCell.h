...
{
public:
  ListCell(const GroupCell *group, Configuration **config, std::unique_ptr<Cell> &&inner);
  ListCell(const GroupCell *group, const ListCell &cell);
  virtual std::unique_ptr<Cell> Copy(const GroupCell *group) const override;
